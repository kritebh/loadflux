...
- **Custom time ranges** — from/to date-time on System, Endpoints, App metrics, and Errors (default rolling last hour)
- **Search & filters** — filter endpoints and slow requests by text; filter errors by status and search the log
- **Multi-instance (cluster)** — aggregate live metrics across replicas with MongoDB; see [Multi-instance deployment](/docs/guides/multi-instance)
- **Ops-friendly config** — optional `disableOnLocalhost`, `listenHost`, and `trustProxy` for deploy and reverse proxies
